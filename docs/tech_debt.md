...
- **Unused `trace` feature**: `Cargo.toml` enables `tower-http/trace` but no `TraceLayer` is ever added. Either drop the feature or wire up request logging — the only server-side observability right now is the thumbnail `eprintln`s.
